...
- **Desktop integration** (GAP-004, 10/10 macOS): `systemTheme`, binary clipboard (`readClipboard`/`writeClipboard`, 64 MiB), `setDockBadge`, `setApplicationMenu`, `setApplicationIcon`, `createTrayItem`, `registerGlobalShortcut` (Carbon), `showNotification`, `WindowHandle.showContextMenu`, `WindowHandle.onFileDrop`. **macOS-verified** via `DesktopProbe`: theme cross-checked vs `AppleInterfaceStyle`, clipboard SHA-256 round-trip, menu/tray/icon structurally self-checked, shortcut `RegisterEventHotKey` noErr, notification deliver-call, file-drop subclass created + web view still renders/IPCs. GUI gestures (clicks, notification banner, context-menu popup, drop) honestly not auto-tested.
- **Deep-link + packaging** (GAP-005): `InfoPlistCustomizer` injects `CFBundleURLTypes` + usage descriptions into a jpackage `Info.plist` (idempotent, XML-escaped); `JpackageArguments` `--icon`/`--file-associations`; `PlatformApplication.setOpenUrlHandler` + macOS `JDeskAppDelegate` (`application:openURLs:`) → activation handler; Gradle `jdesk { deepLink; appIcon; fileAssociation(...) }`. Unit-tested + **verified on a real jpackage Info.plist** (valid XML, idempotent). OS-level scheme/association routing needs a signed installed bundle (BLOCKED in dev).
...
- Window operations: focus/minimize/maximize/fullscreen/always-on-top/min-size/remembered-bounds done; application icon now done (GAP-004 — see Resolved).
- Native desktop integration (GAP-004) is done 10/10 on macOS — see Resolved. GUI-gated behaviours (menu/tray/hotkey *activation*, notification *banner*, context-menu popup, file-drop *gesture*) are implemented but honestly not auto-tested.
